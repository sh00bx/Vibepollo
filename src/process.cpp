  ...
  int proc_t::running() {
    ...
    if (_deferred_launch) {
      ...
      std::thread([this]() {
        ...
        // NB: upstream's parameter is stream_lifecycle_lock_held (no default),
        // not our former terminate_on_failure. This worker is detached and
        // holds no stream lifecycle lock, so it must pass false.
